...
const EnemyFactory = {
  ...
  createGhost(x, y, difficultyMult = 1) {
    const g = new Enemy(x, y, {
      ...
      damage: 8,                               // Low contact damage (it's a shooter)
      radius: 16,
      xpValue: 2
    });
    g.shootCooldown = 0;
    g.shootInterval = 2.5;
    g.orbDamage = Math.floor(12 * difficultyMult);
    g.preferRange = 250;  // Tries to keep distance
    return g;
  },
  ...
  createBat(x, y, difficultyMult = 1) {
    const b = new Enemy(x, y, {
      type: 'bat',
      health: Math.floor(1), // Very low HP
    ...
    b.contactCooldownMax = 2.0; // Longer cooldown so it doesn't spam silence
    return b;
  }
};
