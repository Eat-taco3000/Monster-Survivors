...
class Game {
  ...
  resume(reason = null) {
    ...
    if (reason && this.pauseReason !== reason) return;
    this.paused = false;
    this.pauseReason = null;
    this.lastTime = performance.now();
  }
  ...
  update(dt) {
    ...
    if (this.bloodBoltHealCooldown > 0) {
      this.bloodBoltHealCooldown = Math.max(0, this.bloodBoltHealCooldown - dt);
    }
        ...
        if (proj.collidesWith(enemy)) {
          ...
          if (proj.type === 'bloodBolt') {
            if (this.bloodBoltHealCooldown <= 0) {
              ...
              this.bloodBoltHealCooldown = 0.25;
