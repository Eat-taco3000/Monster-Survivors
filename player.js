...
class Player extends Entity {
  ...
  get effectiveSpeed() {
    const sprintMultiplier = this.isSprinting ? this.sprintSpeedMultiplier : 1;
    return this.speed * this.speedMultiplier * sprintMultiplier;
  }
  ...
  update(dt, game) {
    ...
    this.isSprinting = wantsSprint && this.sprint > 0;

    if (this.isSprinting) {
      this.sprint = Math.max(0, this.sprint - dt);
      if (this.sprint <= 0) {
        this.isSprinting = false;
      }
    } else if (!wantsSprint) {
      this.sprint = Math.min(this.maxSprint, this.sprint + this.sprintRegenRate * dt);
    }
  ...
  takeDamage(amount, game) {
    if (this.invulnTimer > 0 || this.isSprinting) return;
  ...
  draw(ctx) {
    ...
    // Sprint phase aura
    if (this.isSprinting) {
      const sprintPulse = Math.sin(performance.now() / 100) * 0.2 + 0.8;
      ctx.save();
      ctx.globalAlpha = sprintPulse * 0.7;
      ctx.strokeStyle = '#66ddff';
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.arc(sx, sy, this.radius + 8, 0, Math.PI * 2);
      ctx.stroke();
      ctx.globalAlpha = sprintPulse * 0.25;
