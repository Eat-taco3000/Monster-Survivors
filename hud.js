...
const HUD = {
  draw(ctx, game) {
    ...
    // --- Sprint Bar ---
    const sprintBarY = xpBarY + 15;
    const sprintPercent = player.sprint / player.maxSprint;

    ctx.fillStyle = '#1a1a2e';
    ctx.fillRect(hpX, sprintBarY, hpBarWidth, 10);

    ctx.fillStyle = player.isSprinting ? '#66ddff' : '#44ccaa';
    ctx.fillRect(hpX, sprintBarY, hpBarWidth * sprintPercent, 10);

    ctx.strokeStyle = '#333355';
    ctx.lineWidth = 1;
    ctx.strokeRect(hpX, sprintBarY, hpBarWidth, 10);

    ctx.fillStyle = '#bdefff';
    ctx.font = '9px "Segoe UI", sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText(
      `${player.sprint.toFixed(1)} / ${player.maxSprint.toFixed(1)} Sprint`,
      hpX + hpBarWidth / 2,
      sprintBarY + 8
    );
    ...
    for (let i = 0; i < WeaponSystem.weapons.length; i++) {
      ...
      if (weapon.type === 'fireWand') {
        ...
        ctx.fillText(`Dmg: ${Math.round(weapon.damage)} | Chain: ${Math.round(weapon.chainFalloff * 100)}%`, padding + 6, wy + 10);
    ...
    if (game.gameTime < 10) {
      ...
      ctx.fillText('Hold Shift to sprint and phase', canvasWidth / 2, ctx.canvas.height - 14);
